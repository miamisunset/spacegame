...
impl Plugin for UiPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            Update,
            (update_order_queue_overlay, handle_right_click_spawn_menu),
        );
    }
}
    ...
    bsn! {
        ...
        Children [
            ...
            (
                ...
                Children [
                    (
                        Text("WASD: pan  Q/E: yaw  R/F: pitch  Wheel: zoom  RMB drag: orbit  1-4/Space: orders")
...
fn context_button(label: &'static str, kind: OrderKind) -> impl Scene {
    ...
    bsn! {
        ...
        on(move |ev: On<Pointer<Click>>, mut ctx: ResMut<ContextState>, mut ships: Query<&mut OrderQueue, With<ShipStats>>, asteroids: Query<Entity, With<Asteroid>>| {
            if ev.button != PointerButton::Primary {
                return;
            }
            let Some(mut queue) = ships.iter_mut().next() else { return; };
            let target = ctx.target.or_else(|| asteroids.iter().next());
            match kind {
                OrderKind::FlyTo => {
                    let dest = Vec3::new(ctx.screen_pos.x - 400.0, 0.0, ctx.screen_pos.y - 300.0);
                    queue.push_back(Order::FlyTo(dest));
                }
                OrderKind::Approach => {
...
fn handle_right_click_spawn_menu(
    ...
    mut ships: Query<&mut OrderQueue, With<ShipStats>>,
) {
    ...
    if keyboard.just_pressed(KeyCode::Digit1)
        && let Some(mut q) = ships.iter_mut().next()
    {
        q.push_back(Order::FlyTo(Vec3::new(2000.0, 0.0, 0.0)));
    }
    if keyboard.just_pressed(KeyCode::Digit2)
        && let (Some(mut q), Some(target)) = (ships.iter_mut().next(), asteroids.iter().next())
    {
    ...
    if mouse_button.just_pressed(MouseButton::Right) {
        if let Some(pos) = cursor {
            ctx.screen_pos = pos;
            ctx.target = asteroids.iter().next();
            ctx.visible = true;
...
fn update_order_queue_overlay(
    ...
    if queue.is_empty() {
        lines.push("Queue: [empty] - Right-click or 1-4/Space".to_string());
    } else {
